...
- on select question 
   -- select all answers from table `answers` which satisfy at least one word of selected question, set `numOf_Fixed_clicks` for them
   -- append answers from question assigned-answers
   -- order all answers by numOf_Fixed_clicks dec
   
   -- display one of the answers with two buttons `Fixed` and `Not Fixed`
   -- on click to `Not Fixed` show next `answer`, 
   -- on click on 'Fixed', if row exists in table `question-answers`increment incremnet `numOf_Fixed_clicks`, otherwise create a new row in table `question-answers`
